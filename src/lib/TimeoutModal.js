...
function utloptTidspunktMinusFemMinutter(remainingSeconds) {
    return (remainingSeconds - 300) * 1000;
}

const TimeoutModal = props => {
    const { visDemo, authUrl } = props
    const [skalVise, setSkalVise] = useState(visDemo)
        ...
        fetch(authUrl || '/api/auth',
            ...
            .then(authExp => {
                ...
                if (remainingSeconds) {
                    ...
                    // SetTimeout til å vise informasjonsmelding når sesjon er utløpt
                    setTimeout(() => {
                        setSkalVise(true)
                    }, expirationInMillis);
